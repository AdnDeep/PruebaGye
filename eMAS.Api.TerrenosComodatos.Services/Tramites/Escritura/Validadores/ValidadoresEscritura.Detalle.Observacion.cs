...
namespace eMAS.Api.TerrenosComodatos.Services
{
    ...
    {
        public bool ValidarDatosClienteTramiteObservacionEditViewModel(ref ObservacionTramiteEditViewModel entrada
            , ref ResultadoDTO<int> salida)
        {
            var parametros = $"ValidadoresEscrituraTramite Service Layer";
            var props = new Dictionary<string, object>(){
                                { "Metodo", "ValidarDatosClienteTramiteObservacionEditViewModel" },
                                { "Sitio", "COMODATO-API" },
            ...
            if (string.IsNullOrEmpty(entrada.observacion) || string.IsNullOrWhiteSpace(entrada.observacion))
            {
                ...
                {
                    codigo = "VLNVALCLI",
                    descripcion = "Indicar una observacion es obligatorio.",
                    tipo = "ADVERTENCIA"
                });
                salida.mensajes = lsMensajes;
                salida.mensaje = "Indicar una observacion es obligatorio.";
        ...
        public bool ValidarRespuestaServidorTramiteObservacionAccionAgregar(ref Tuple<short, string> entrada
            , ref ResultadoDTO<int> salida) 
        {
            var parametros = $"ValidadoresEscrituraTramite Service Layer";
            var props = new Dictionary<string, object>(){
                                { "Metodo", "ValidarRespuestaServidorTramiteObservacionAccionAgregar" },
                                { "Sitio", "COMODATO-API" },
                                { "Parametros", parametros }
                        };
            bool puedeContinuar = false;
        ...
        public bool ValidarRespuestaServidorTramiteObservacionAccionActualizar(ref Tuple<short, string> entrada
            , ref ResultadoDTO<int> salida)
        {
            var parametros = $"ValidadoresEscrituraTramite Service Layer";
            var props = new Dictionary<string, object>(){
                                { "Metodo", "ValidarRespuestaServidorTramiteObservacionAccionActualizar" },
                                { "Sitio", "COMODATO-API" },
                                { "Parametros", parametros }
                        };
            bool puedeContinuar = false;
