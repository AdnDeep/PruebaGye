    ...
    public partial class ValidadoresEscrituraTramite
    {
            ...
            , ref ResultadoDTO<int> salida)
        {
            ...
            var props = new Dictionary<string, object>(){
                                { "Metodo", "ValidaRespuestLogicDataValidationEscrituraTramiteOficioServidor" },
                                { "Sitio", "COMODATO-API" },
            ...
            if (idoficiotramite > 0) 
            {
                ...
                if (existeOficioTramiteTmp == null)
                {
                    using (_logger.BeginScope(props))
                    {
                        _logger.LogError($"La clave EXISTEOFICIOTRAMITE no se encuentra en la BD.");
                    }
                    salida.mensaje = "Se produjo un error Interno en la aplicación. (7)";
                    salida.tipo = "ADVERTENCIA";
                    return puedeContinuar;
                }
                if (!existeOficioTramiteTmp.VALORBOOLEANO)
                {
                    lsMensajes.Add(new Mensaje
                    {
                        codigo = "VLNVALSERV",
                        descripcion = "El Oficio trámite indicado no existe en el sistema o está dado de baja.",
                        tipo = "ADVERTENCIA"
                    });
                    salida.mensajes = lsMensajes;
                    salida.mensaje = "El Oficio trámite indicado no existe en el sistema o está dado de baja.";
                    salida.tipo = "ADVERTENCIA";
                    return puedeContinuar;
                }
            }
