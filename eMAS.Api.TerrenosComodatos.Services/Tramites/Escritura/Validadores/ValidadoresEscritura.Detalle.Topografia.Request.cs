...
namespace eMAS.Api.TerrenosComodatos.Services
{
    ...
    {
        public bool TopografiaDataRequestToAdd(ref TopografiaTerrenoEditViewMoel entrada
            ...
            , ref ResultadoDTO<int> salida) 
        {
            ...
            var props = new Dictionary<string, object>(){
                                { "Metodo", "TopografiaDataRequestToAdd" },
                                { "Sitio", "COMODATO-API" },
            ...
            if (entrada.idtopografiaterreno != 0)
            {
            ...
            if (entrada.idtipotopografiaterreno <= 0)
            {
                salida.mensaje = "El parámetro de Entrada Id Tipo Topografia del tramite debe ser mayor a 0";
                salida.tipo = "ADVERTENCIA";
                return puedeContinuar;
            }
        ...
        public bool TopografiaRequestToUpdate(ref TopografiaTerrenoEditViewMoel entrada
            , string usuario, string controlador, string pcclient
            , ref ResultadoDTO<int> salida)
        {
            var parametros = $"ValidadoresEscrituraTramite Service Layer";
            var props = new Dictionary<string, object>(){
                                { "Metodo", "TopografiaRequestToUpdate" },
                                { "Sitio", "COMODATO-API" },
                                { "Parametros", parametros }
                        };
            List<Mensaje> lsMensajes = new List<Mensaje>();
