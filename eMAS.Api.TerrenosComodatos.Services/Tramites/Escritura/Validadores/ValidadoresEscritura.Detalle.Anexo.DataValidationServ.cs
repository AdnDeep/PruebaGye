...
namespace eMAS.Api.TerrenosComodatos.Services
{
    ...
    {
        public bool ValidaRespuestLogicDataValidationEscrituraTramiteAnexoServidor(int idAnexoTramite, int idTramite, ref Tuple<List<SmcValidaDataServidor>,string> entrada
            , ref ResultadoDTO<int> salida)
        {
            ...
            if (idAnexoTramite > 0) 
            {
                ...
                if (idTramite > 0)
                {
                    var existeRelacionAnexoTramiteTmp = lsValidacion.FirstOrDefault(fod => fod.CLAVE == "EXISTERELACION");
                    if (existeRelacionAnexoTramiteTmp == null)
                    {
                        using (_logger.BeginScope(props))
                        {
                    ...
                    if (!existeRelacionAnexoTramiteTmp.VALORBOOLEANO)
                    {
                        lsMensajes.Add(new Mensaje
                        {
                            codigo = "VLNVALSERV",
                            descripcion = "El Anexo no tiene relación con el trámite en el sistema o está dado de baja.",
                            tipo = "ADVERTENCIA"
                        });
                        salida.mensajes = lsMensajes;
                        salida.mensaje = "El Anexo no tiene relación con el trámite en el sistema o está dado de baja.";
                        salida.tipo = "ADVERTENCIA";
                        return puedeContinuar;
                    }
                }
